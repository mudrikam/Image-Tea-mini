...
# Import the donation dialog function directly from the same place the status bar uses
try:
    from core.helper.dialogs._donation_dialog import show_donation_dialog
except ImportError:
    print("Donation dialog module not found")
...
class ActionsWidget:
    ...
    def _setup_components(self):
        ...
        if self.widget and qta:
            ...
            if donate_button:
                ...
                # Connect click event but defer getting config until clicked
                if show_donation_dialog is not None:
                    donate_button.clicked.connect(self._on_donate_button_clicked)
                else:
    ...
    def _show_donation_dialog(self):
        ...
        if show_donation_dialog is not None and self.main_window is not None and self.config:
            ...
            print("Warning: Cannot show donation dialog - missing main window, config, or dialog function")
    ...
    def _on_donate_button_clicked(self):
        """Handle donate button click - get fresh config and show dialog."""
        if show_donation_dialog is not None:
            # Get main window
            main_window = None
            ...
            while parent_widget:
                ...
                if parent_widget and isinstance(parent_widget, QtWidgets.QMainWindow):
                    main_window = parent_widget
                    break
            
            # Get fresh config from AppConfig
            try:
                from core.utils.app_config import AppConfig
                config = AppConfig.get_instance().config
                # Show the dialog with the fresh config
                show_donation_dialog(main_window, config, self.BASE_DIR)
            except ImportError:
                print("Warning: AppConfig not found, using stored config")
                # Fall back to stored config if available
                if self.config and main_window:
                    show_donation_dialog(main_window, self.config, self.BASE_DIR)
                else:
                    print("Warning: Cannot show donation dialog - missing config or main window")
        else:
            print("Warning: Donation dialog function not available")
